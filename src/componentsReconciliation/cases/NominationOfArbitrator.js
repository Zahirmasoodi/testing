...
const NominationOfArbitrator = (props) => {
  ...
  const enableNextStep = (id) => {
    ...
    stepForm.append("step", 4);
    stepForm.append("stepName", "transmission");
    axios
  ...
  const handleUpload = (file) => {
    ...
    form.append("type", "nominationOfArb");
    ...
    axios
      ...
      .then(() => {
        ...
        notice.append(
          "notice",
          `File Uploaded to RFA by ${props.location.state.claimantName}`
        );
        axios
          .post(`${baseURL}/notify/new/${props.location.state._id}`, notice)
  ...
  const docs = () => {
    return props.location.state.attachments.map((attach) => {
      if (attach.type == "nominationOfArb") {
        index += 1;
        return (
          <tr key={attach._id}>
          ...
          <Col>
            ...
            <Card style={styles.cardBg}>
              <h6 className="pl-2" style={styles.center}>
                III. Nomination &amp; Appointment Of Arbitrator
