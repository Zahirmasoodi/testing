...
const TransmissionOfArbitrationFile = (props) => {
  ...
  const enableNextStep = (id) => {
    ...
    stepForm.append("step", 5);
    stepForm.append("stepName", "premliminary");
    axios
  ...
  const handleUpload = (file) => {
    ...
    form.append("type", "timelineOfProceeding");
    axios
      .post(
  ...
  const docs = () => {
    return props.location.state.attachments.map((attach) => {
      if (attach.type == "timelineOfProceeding") {
        index += 1;
        return (
          <tr key={attach._id}>
          ...
          <Col>
            ...
            <Card style={styles.cardBg}>
              <h6 className="pl-2" style={styles.center}>
                IV. Transmission Of File to Conciliator(s)
...
export default TransmissionOfArbitrationFile;
